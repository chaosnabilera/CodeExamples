...
def parse_ipheader(data):
	ipheader = struct.unpack('!BBHHHHH4s4s', data[:20])
	# B
	version = (ipheader[0] & 0xF0) >> 4
	header_length = (ipheader[0] & 0xF)
	# B
	...
	# H
	time_to_live = (ipheader[5] & 0xFE00) >> 9
	protocol = (ipheader[5] & 0x1FF)
	# H
	header_checksum = ipheader[6]
	# 4s
	source_ipaddress = ipheader[7]
	source_ipaddress = socket.inet_ntoa(source_ipaddress)
	# 4s
	destination_ipadress = ipheader[8]
